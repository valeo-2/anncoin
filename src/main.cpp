...
const string strMessageMagic = "AnnCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 60; // AnnCoin: 1 days
static const int64 nTargetSpacing = 60; // AnnCoin: 1 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal AnnCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AnnCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1403450323; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0;

        if (fTestNet)
        {
            block.nTime    = 1403450323;
            block.nNonce   = 0;
        }
